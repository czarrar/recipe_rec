...
class Tfidf_Wrapper(object):
    def __init__(self):
        # For removing punctuation and digits from a document
        #self._table = str.maketrans('', '', string.punctuation + string.digits + [''])
    ...
    def _tokenize(self, text):
        ...
        tokens = [ item for item in tokens if len(item) > 1 if not item in self._stop_words ]
